...
| RNF-03 | Seguridad | Los datos sensibles deben transmitirse sobre HTTPS | Certificado | ⚠️ | Preparado para HTTPS: cookies con flag `httponly=True`, Lighthouse CI omite `is-on-https` para entorno local. En producción se configura TLS via reverse proxy (Nginx/ALB). No implementado en entorno local de desarrollo. |
