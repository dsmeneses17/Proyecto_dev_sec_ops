...
| RNF-03 | Seguridad | Los datos sensibles deben transmitirse sobre HTTPS | Certificado | ✅ | `infra/nginx/` – Reverse proxy Nginx con TLS (TLSv1.2/1.3). Certificado autofirmado generado automáticamente (`entrypoint.sh` con `openssl`). Redirect HTTP→HTTPS en puerto 80. Headers de seguridad: `Strict-Transport-Security`, `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`. `AppRestaurante/app/main.py` – middleware `enforce_https_middleware()`. Cookie flags: `secure=True`, `httponly=True`. Docker Compose expone solo puertos 80/443 vía `secure_gateway`. |
| RNF-04 | Seguridad | Los endpoints deben implementar rate limiting | 100 req/min | ✅ | `ApiRestaurante/app/main.py` y `AppRestaurante/app/main.py` – `RateLimitMiddleware` usando librería `limits==3.14.1` con `MovingWindowRateLimiter` + `MemoryStorage`. Límite: 100 req/min por IP. Respuesta 429 con mensaje `"Rate limit exceeded: 100 per 1 minute"`. Commit `52f3762`. |
