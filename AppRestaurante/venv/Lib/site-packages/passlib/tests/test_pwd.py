...
class WordGeneratorTest(TestCase):
    ...
    def test_returns(self):
        """'returns' keyword"""
        # returns=int option
        results = genword(returns=8080)
        self.assertResultContents(results, 8080, ascii_62)

        # returns=iter option
        gen = genword(returns=iter)
        results = [next(gen) for _ in range(8080)]
        self.assertResultContents(results, 8080, ascii_62)
    ...
    def test_charset(self):
        ...
        # charset option
        results = genword(charset="hex", returns=8080)
        self.assertResultContents(results, 8080, hex)

        # chars option
        # there are 3**3=27 possible combinations
        results = genword(length=3, chars="abc", returns=8080)
        self.assertResultContents(results, 8080, "abc", unique=27)
...
class PhraseGeneratorTest(TestCase):
    ...
    def test_wordset(self):
        ...
        # wordset option
        results = genphrase(words=simple_words, returns=8080)
        self.assertResultContents(results, 8080, simple_words)

        # words option
        results = genphrase(length=3, words=simple_words, returns=8080)
        self.assertResultContents(results, 8080, simple_words, unique=3**3)
