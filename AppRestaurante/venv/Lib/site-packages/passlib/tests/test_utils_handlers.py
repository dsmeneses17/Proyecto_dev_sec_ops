...
class SkeletonTest(TestCase):
    ...
    def test_91_parsehash(self):
        ...
        self.assertEqual(result, dict(salt=s, rounds=8080,
                                      implicit_rounds=True, checksum=c))

        # omit checksum
        result = hash.sha256_crypt.parsehash(h, checksum=False)
        self.assertEqual(result, dict(salt=s, rounds=8080, implicit_rounds=True))

        # sanitize
        result = hash.sha256_crypt.parsehash(h, sanitize=True)
        self.assertEqual(result, dict(rounds=8080, implicit_rounds=True,
            salt=u('LK**************'),
             checksum=u('U0pr***************************************')))
