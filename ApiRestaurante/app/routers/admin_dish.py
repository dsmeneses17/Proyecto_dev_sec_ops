...
async def list_dishes_by_category(user=Depends(get_current_user), db: Session = Depends(get_db)):
    from app.models.category import Category
    from app.models.dish import Dish
    ...
    categorias = db.query(Category).filter(Category.restaurante_id == restaurant_id).all()
    result = []

    for cat in categorias:
        platos = db.query(Dish).filter(Dish.categoria_id == cat.id, Dish.eliminado_en == None).all()
...
async def list_dishes(user=Depends(get_current_user), db: Session = Depends(get_db)):
    return db.query(Dish).filter(Dish.eliminado_en == None).all()


# Obtener plato por ID
@router.get("/{dish_id}", response_model=DishOut)
async def get_dish(dish_id: UUID, user=Depends(get_current_user), db: Session = Depends(get_db)):
...
async def update_dish(
    dish_id: UUID, payload: DishUpdate, user=Depends(get_current_user), db: Session = Depends(get_db)
):
    dish = db.query(Dish).filter(Dish.id == dish_id, Dish.eliminado_en == None).first()
    if not dish:
        raise HTTPException(status_code=404, detail="Plato no encontrado")
